...
ZENO_API PostProcPass::PostProcPass(): RenderPassBase() {}
ZENO_API PostProcPass::PostProcPass(const std::string &name)
    : RenderPassBase(name) {}
ZENO_API PostProcPass::~PostProcPass() = default;

ZENO_API size_t PostProcPass::serializeSize() const {
    size_t size{0};
...
ZENO_API std::vector<char> PostProcPass::serialize() const {
    std::vector<char> str;
    str.resize(serializeSize());
    serialize(str.data());
    return str;
...
ZENO_API void PostProcPass::serialize(char *str) const {
    size_t i{0};
...
ZENO_API PostProcPass PostProcPass::deserialize(std::vector<char> const &str) {
    PostProcPass renderPass;
...
ZENO_API void PostProcPass::render() {
    std::cout << "Render PostProcPass " << name << std::endl;
}
